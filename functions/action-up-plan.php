...
if (!in_array($tabela, ['contatos', 'leads'])) {
  ...
  header("Location: ../dashboard&page=clients-up");
  exit;
}
...
if (
  ...
  if ($handle) {
    ...
    if ($tabela === 'contatos') {
      // Coleta todas as situações distintas com contagem
      $resultado = $db->query("SELECT status, COUNT(*) as total FROM contatos GROUP BY status");
    
      $situacoes = [];
      while ($row = $resultado->fetchArray(SQLITE3_ASSOC)) {
        $situacoes[$row['status']] = (int) $row['total'];
      }
    
      $_SESSION['situacoes'] = $situacoes;
...
$destino = $tabela === 'leads' ? 'dashboard&page=leads-up' : 'dashboard&page=clients-up';
